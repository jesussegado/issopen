# Issopen self-hosting and operations

This is the technical guide for running, developing, validating and operating
Issopen. For the product overview, return to the [project README](../README.md).
...
as described in [invitation operations](invitation-delivery.md); mail is
...
[privacy policy and data contract](privacy.md) explains retention,
...
See [delivery and operation](chrome-delivery.md). Existing read-only
...
[approved Epic scope](chrome-extension.md). `pnpm extension:validate`
...
paleta verde bosque/menta. La [decisión de diseño](design/0001-brand-identity.md)
...
[Epic archiving](epic-archiving.md) for API, MCP and rollback details.
...
idempotent issue creation. See [Epic 5 frontend delivery](epic-5-frontend.md).
...
are in [agent-onboarding.md](agent-onboarding.md).
...
are documented in [agent-credentials.md](agent-credentials.md).
...
[Google OpenID Connect](google-oauth.md).
