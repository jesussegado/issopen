...
describe("tracker web routes", () => {
  it("renders server text safely and moves a card only after the authoritative response", async () => {
    ...
    expect(screen.getByRole("link", { name: issue.title })).toHaveTextContent(
    ...
    const status = screen.getByRole("combobox", {
      name: "Change status for PRI-1",
    });
    await user.selectOptions(status, "ready");
    expect(await screen.findByText("PRI-1 moved to Ready")).toBeInTheDocument();
    await waitFor(() =>
      expect(
        screen.getByRole("combobox", { name: "Change status for PRI-1" }),
      ).toHaveFocus(),
    );
    expect(
  ...
  it("collapses board columns and expands card previews independently", async () => {
    ...
    expect(screen.getByText(issue.key)).toBeVisible();
    ...
    expect(screen.getByText("First line", { exact: false })).toBeVisible();
    expect(
      screen.getByRole("button", { name: "Hide details for PRI-1" }),
  ...
  it("marks and filters cards with unanswered blocking questions", async () => {
    ...
    expect(screen.queryByText(clear.title)).not.toBeInTheDocument();
  ...
  it("renders immutable, attributed activity and review controls from the server", async () => {
    ...
    expect(screen.getByText("Created PRI-1")).toBeInTheDocument();
    const activityItems = document.querySelectorAll(
      ".activity-list .activity-item",
    );
    expect(activityItems).toHaveLength(2);
    expect(activityItems[0]).toHaveTextContent("Accepted PRI-1");
    expect(activityItems[1]).toHaveTextContent("Created PRI-1");
  ...
  it("navigates questions and saves option or Other answers", async () => {
    ...
    const detailStatus = screen.getByLabelText("Change status for PRI-1");
    expect(
